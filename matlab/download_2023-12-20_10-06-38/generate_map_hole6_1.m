clear all;
%% Set up data
fix = readmatrix("20230919/hole6_2-novatel-fix");
fix_sand = readmatrix("csv/h6_ob29_sand-novatel-fix");
image_name = "route6_resolution05_rtk_1001.png";
...
%% 
llh_sand = fix_sand(:,8:10);
for i = 1:size(llh_sand,1)
    enu_sand(i,:) = llh2enu(llh_sand(i,:), ee_building_llh);
end
pgon_sand = polyshape(enu_sand(:,1),enu_sand(:,2));
plot(pgon_sand)
...
for i = 1:rows
    for j = 1:cols
        ...
        if(isinterior(pgon_sand,pixel_center(1),pixel_center(2)))
            grid_map(i, j) = 0;
        end        
    end
end
